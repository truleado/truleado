-- Safe migration for Chat & Find feature with usage tracking
...
-- 3. Add usage tracking columns to profiles table (if they don't exist)
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'profiles' AND column_name = 'chat_find_searches_count') THEN
        ALTER TABLE profiles ADD COLUMN chat_find_searches_count INTEGER DEFAULT 0;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'profiles' AND column_name = 'chat_find_free_searches_used') THEN
        ALTER TABLE profiles ADD COLUMN chat_find_free_searches_used INTEGER DEFAULT 0;
    END IF;
END $$;
...
DROP POLICY IF EXISTS "Users can view their own search history" ON chat_find_searches;
DROP POLICY IF EXISTS "Users can insert their own searches" ON chat_find_searches;
DROP POLICY IF EXISTS "Users can update their own searches" ON chat_find_searches;
DROP POLICY IF EXISTS "Users can view their own search results" ON chat_find_results;
DROP POLICY IF EXISTS "Users can insert their own search results" ON chat_find_results;

-- 7. Create RLS policies for chat_find_searches
...
-- 8. Create RLS policies for chat_find_results
...
-- 9. Drop existing functions if they exist and recreate them
DROP FUNCTION IF EXISTS can_perform_chat_find_search(UUID);
DROP FUNCTION IF EXISTS increment_chat_find_search_count(UUID);
DROP FUNCTION IF EXISTS get_chat_find_usage_info(UUID);

-- 10. Create function to check if user can perform chat find search
...
-- 11. Create function to increment chat find search count
...
-- 12. Create function to get user usage info
...
-- 14. Update existing users to have 0 free searches used (if column was just added)
UPDATE profiles 
SET chat_find_free_searches_used = 0 
WHERE chat_find_free_searches_used IS NULL;
