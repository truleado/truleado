...
// Working Zoho contact creation function
async function createZohoContact(email: string, fullName: string) {
  try {
    console.log('🔗 Creating Zoho contact for:', email, 'name:', fullName)
    ...
    if (!tokenResponse.ok) {
      console.warn('Failed to get Zoho access token')
      return
    }
    ...
    if (contactResponse.ok) {
      ...
      console.log('✅ Zoho contact created successfully:', result.data[0]?.details?.id)
    } else {
      const errorText = await contactResponse.text()
      console.warn('Failed to create Zoho contact:', errorText)
    }
  } catch (error) {
    console.warn('Zoho contact creation failed:', error)
  }
}
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6 sm:p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex items-start">
              ...
              <label htmlFor="terms" className="ml-2 block text-sm text-gray-700">
                I agree to the{' '}
                <Link href="/terms" className="text-[#148cfc] hover:text-[#0d7ce8]">
                  Terms of Service
                </Link>{' '}
                and{' '}
                <Link href="/privacy" className="text-[#148cfc] hover:text-[#0d7ce8]">
                  Privacy Policy
                </Link>
              </label>
            </div>
          ...
          <div className="mt-6">
            <button
              ...
              disabled={loading}
              className="w-full flex items-center justify-center px-4 py-3 border border-gray-300 rounded-lg shadow-sm bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#148cfc] disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
          ...
          <div className="mt-6 text-center">
            <p className="text-gray-600">
              ...
              <Link href="/auth/signin" className="text-[#148cfc] hover:text-[#0d7ce8] font-semibold">
                Sign in
              </Link>
            </p>
          </div>
